...
class Pass(models.Model):
    PLAN_TYPE = (('all_programs', 'all programs'), ('only_private_lesson', 'only private lesson'), ('only_public_class', 'only public class'))
    ...
    pass_type = models.CharField(choices=PLAN_TYPE, null = False, max_length=255) # 수강권 종류 - 수강 가능한 프로그램 종류
    duration_months = models.IntegerField() # 수강권 유효기간 (개월 단위)
    price = models.IntegerField() # 가격
    monthly_revenue = models.IntegerField() # 월 단위로 분할된 가격
...
class Member(models.Model):
    ...
    renewal_count = models.IntegerField(null=False, default=0) # 연장 횟수

    def __str__(self):
        return self.id

class Payments(models.Model):
    PAYMENT_METHOD = (('online', '온라인 결제'), ('offline', '오프라인 결제'))
    ...
    user_id = models.CharField(max_length=200, null = False, blank = True) # 사용자 또는 수강생과 연결된 외래 키 (Foreign Key)
